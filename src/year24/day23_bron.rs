...
pub(crate) fn solve(data: &str) -> (String, String) {
    ...
    let tl=triples.len();
    let mut candidates = triples.into_iter().unique().collect_vec();
    assert_eq!(candidates.len(), tl);
    ...
    while candidates.len() > 0 {
        ...
        for candidate in candidates.iter() {
            ...
            for &j in connections[curr_ind].iter() {
                let connected = candidate.iter().all(|&c| (connections[c].contains(&j)) || (connections[j as usize].contains(&(c as u16))));
                if connected {
